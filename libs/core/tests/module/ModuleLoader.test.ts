...
describe('ModuleLoader test suite', () => {
    it('should extract modules to directory', async () => {
        ...
        const onError = vi.fn();
        const moduleLoader = new ModuleLoader(join(__dirname, './test_files/module1'));
        await moduleLoader.extractModule(join(__dirname, './test_files/module1.zip'), true)
            .then(onFinish)
            .catch(onError);

        expect(onFinish).toHaveBeenCalled();
        expect(onError).not.toHaveBeenCalled();
        expect(listFilesRecursively(join(__dirname, './test_files/module1')).length).toEqual(3);
    });
    ...
    it('should return false when files integrity is invalid', async () => {
        const moduleLoader = new ModuleLoader(join(__dirname, './test_files/module_invalid_hash'));
        await moduleLoader.extractModule(join(__dirname, './test_files/module_invalid_hash.zip'), true);
        const integrity = moduleLoader.checkModuleIntegrity();
    ...
    it('should successfully register routes', async () => {
        ...
        const moduleRoutes: RouteObject[] = [
            ...
            },
            {
                ...
                element: createElement(RedirectToMainModuleRoute, { moduleRouteIdentifier: 'frmeastmodule1' }),
                children: [
                    {
                        path: '',
                        element: createElement('h1')
